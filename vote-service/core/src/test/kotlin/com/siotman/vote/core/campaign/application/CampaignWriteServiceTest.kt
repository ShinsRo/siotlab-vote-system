...
class CampaignWriteServiceTest {
    private val campaignRepository = TestCampaignRepository()
    private val campaignWriteService = CampaignWriteService(campaignRepository)
    ...
    fun `관리자가 캠페인을 생성하면 DRAFT 상태로 저장된다`() {
        ...
        val created = campaignWriteService.create(command)
            .block()!!
    ...
    fun `관리자가 캠페인을 활성화하면 ACTIVE 상태가 된다`() {
        val created = campaignWriteService.create(
        ...
        val activated = campaignWriteService.activate(requireNotNull(created.id))
            .block()!!
    ...
    fun `관리자가 캠페인 이름 설명 기간을 수정할 수 있다`() {
        ...
        val updated = campaignWriteService.update(
