...
import com.siotman.vote.voteevent.support.TestVoteEventRepository
import org.assertj.core.api.Assertions.assertThat
import org.junit.jupiter.api.Test
import java.time.LocalDateTime

class VoteEventAdmServiceTest {
    private val voteEventRepository = TestVoteEventRepository()
    private val voteEventAdmService = VoteEventAdmService(voteEventRepository)

    @Test
    fun `관리자가 투표 이벤트를 생성하면 DRAFT 상태로 저장된다`() {
        val created = voteEventAdmService.create(
    ...
    fun `관리자가 투표 이벤트를 활성화할 수 있다`() {
        ...
        val activated = voteEventAdmService.activate(requireNotNull(created.id)).block()!!
    ...
    fun `관리자가 이벤트 이름 설명 기간을 수정할 수 있다`() {
        ...
        val updated = voteEventAdmService.update(
